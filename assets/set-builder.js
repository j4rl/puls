...
export function initSetBuilder({form,draft,loadDraft}){
 ...
 const designEditor=createSetDesignEditor();settings.append(designEditor.element);
 ...
 function draw(){
  ...
  panel.querySelectorAll('[data-index]').forEach(button=>button.addEventListener('click',()=>{capture();index=Number(button.dataset.index);loadDraft(questions[index]);draw();}));
 ...
 panel.querySelector('#set-add').addEventListener('click',()=>{
  if(questions.length>=20)return;capture();questions.push({kind:'choice',title:'',options:['',''],min:0,max:10,view:'bars'});index=questions.length-1;loadDraft(questions[index]);draw();form.querySelector('#question-title').focus();
 });
 for(const [id,delta] of [['set-up',-1],['set-down',1]])panel.querySelector('#'+id).addEventListener('click',()=>{
  const next=index+delta;if(next<0||next>=questions.length)return;capture();[questions[index],questions[next]]=[questions[next],questions[index]];index=next;draw();
 });
 panel.querySelector('#set-remove').addEventListener('click',()=>{
  if(questions.length===1)return;questions.splice(index,1);index=Math.min(index,questions.length-1);loadDraft(questions[index]);draw();form.querySelector('#question-title').focus();
 });
 return {
  ...
  payload(){
   ...
   for(let i=0;i<questions.length;i++){
    const q=questions[i];
    const invalid=!q.title||(['choice','check'].includes(q.kind)&&(q.options.length<2||q.options.some(o=>!o)||new Set(q.options.map(o=>o.toLocaleLowerCase('sv'))).size!==q.options.length))||(q.kind==='number'&&(!Number.isFinite(q.min)||!Number.isFinite(q.max)||q.min>=q.max||Math.abs(q.min)>1000000||Math.abs(q.max)>1000000));
