...
function createPage(){
 ...
 function loadDraft(q){
  ...
  form.elements.title.value=q.title;optionBox.replaceChildren();(q.options.length?q.options:['','']).forEach(addOption);
  optionBox.querySelectorAll('input').forEach(el=>el.disabled=!['choice','check'].includes(q.kind));
