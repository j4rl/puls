...
function createPage(){
 ...
 editorDialog.append(titleField,optionsWrap,rangeWrap,answerHelp,divider,viewStep,viewField,createError);
 const editorActions=document.createElement('div');editorActions.className='dialog-actions';editorActions.innerHTML='<button type="button" class="btn" data-editor-cancel>Avbryt</button><button type="button" class="btn primary" data-editor-save>Spara fråga</button>';editorDialog.append(editorActions);form.append(editorDialog);
 editorActions.querySelector('[data-editor-cancel]').addEventListener('click',()=>editorDialog.close());
 editorActions.querySelector('[data-editor-save]').addEventListener('click',()=>editorDialog.close());
...
async function livePage(){
 ...
 function draw(){document.querySelector('#live-chart').innerHTML=hidden?'<div class="liveempty">Resultatet är dolt.<br><small>Alla kan fortsätta svara i lugn och ro.</small></div>':renderChart(q,answers);const count=document.querySelector('#response-count');if(count.textContent!==`${answers.length} svar`)count.textContent=`${answers.length} svar`;document.querySelector('#pause').textContent=q.open?'Pausa svar':'Öppna för svar';document.querySelector('#live-status').textContent=q.open?'Tar emot svar':'Frågan är pausad';document.querySelector('#stage-status').textContent=q.open?'LIVE':'PAUSAD';document.querySelector('#live-view').value=q.view;applySetDesign(app,{title:q.title,design:q.design});}
 draw();
