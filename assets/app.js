...
const kinds=[['choice','Flerval','Välj ett alternativ','◉'],['yesno','Ja / nej','Ta ställning','±'],['number','Värde','Ange ett tal','#'],['check','Kryssrutor','Välj flera alternativ','☑'],['sentence','Mening','Svara med fri text','≡'],['word','Ord','Ett ord per person','Aa']];
...
function accountControls(){
 ...
 if(settings.user){
  ...
  target.innerHTML='<button class="btn" id="login" type="button">Logga in</button><button class="textbutton" id="register" type="button">Skapa konto</button>';
  target.querySelector('#login').addEventListener('click',()=>openAccount('login'));
  target.querySelector('#register').addEventListener('click',()=>openAccount('register'));
 }
}
...
function createPage(){
 ...
 function draft(){return {kind:activeKind,title:form.elements.title.value.trim(),options:activeKind==='yesno'?['Ja','Nej']:['choice','check'].includes(activeKind)?[...optionBox.querySelectorAll('input')].map(el=>el.value.trim()):[],min:document.querySelector('#min').valueAsNumber,max:document.querySelector('#max').valueAsNumber,view:document.querySelector('#view').value};}
 function preview(){const q=draft();document.querySelector('#max').setCustomValidity(activeKind==='number'&&q.max<=q.min?'Största värdet måste vara större än det minsta.':'');if(q.max<=q.min||!Number.isFinite(q.max)||!Number.isFinite(q.min)){q.min=0;q.max=10;}q.options=q.options.map((s,i)=>s||`Alternativ ${i+1}`);document.querySelector('#preview-title').textContent=q.title||'Vad vill du att vi fokuserar på idag?';const a=sample(q);document.querySelector('#preview-chart').innerHTML=renderChart(q,a);document.querySelector('#sample-count').textContent=`${a.length} exempelsvar`;}
 form.addEventListener('change',event=>{if(event.target.name==='kind'){activeKind=event.target.value;form.querySelectorAll('.kind').forEach(el=>el.classList.toggle('selected',el.querySelector('input').checked));const optionsVisible=['choice','check'].includes(activeKind);document.querySelector('#options-wrap').hidden=!optionsVisible;optionBox.querySelectorAll('input').forEach(el=>el.disabled=!optionsVisible);document.querySelector('#range-wrap').hidden=activeKind!=='number';document.querySelectorAll('#range-wrap input').forEach(el=>{el.disabled=activeKind!=='number';el.required=activeKind==='number';});document.querySelector('#view').innerHTML=viewOptions(activeKind,allowed[activeKind][0]);document.querySelector('#answer-help').textContent={word:'Ett ord per deltagare, högst 40 tecken.',sentence:'Ett textsvar per deltagare, högst 500 tecken.',number:'Deltagaren anger ett tal inom intervallet. Decimaler är tillåtna.',yesno:'Deltagaren väljer Ja eller Nej.',check:'Deltagaren kan välja flera alternativ.',choice:''}[activeKind];}preview();});
 ...
 form.addEventListener('submit',async event=>{event.preventDefault();const button=document.querySelector('#publish'),box=document.querySelector('#create-error');if(button.disabled||!form.reportValidity())return;error(box,'');button.disabled=true;button.textContent='Publicerar…';try{if(!settings.csrf)throw Error(bootError||'Ladda om sidan först.');const result=await api('create',{data:draft()});location.href=urlFor('live',result.code);}catch(err){error(box,err.message);button.disabled=false;button.textContent='Publicera fråga ↗';}});
 preview();if(bootError)error(document.querySelector('#global-error'),bootError);
 document.querySelector('.questions>p').textContent=settings.user?'Dina frågor är sparade på ditt konto och kan öppnas på alla enheter där du loggar in.':'Frågorna hör till den här webbläsaren. Skapa ett konto för att spara dem och öppna dem på andra enheter, eller behåll webbläsarens cookies.';
 loadList();
}
async function loadList(){const target=document.querySelector('#question-list');try{const {questions}=await api('list');target.innerHTML=questions.length?questions.map(q=>`<a class="questionrow" href="${urlFor('live',q.code)}"><div><strong>${e(q.title)}</strong><small>${e(kinds.find(k=>k[0]===q.kind)?.[1]||q.kind)} · Kod ${e(q.code)} · ${Number(q.is_open)?'Öppen':'Pausad'}</small></div><span class="small">${Number(q.answer_count)} svar ↗</span></a>`).join(''):'<div class="emptylist">Dina frågor dyker upp här när du publicerar den första.</div>';}catch(err){error(target,err.message);}}
...
async function joinQuestion(){
 try{
  ...
  const {question:q,answered}=await api('question',{code});if(answered){answeredPage();return;}
  let inputs='';
  ...
  else if(q.kind==='number')inputs=`<label class="field"><span>Ditt värde (${number(q.min)}–${number(q.max)})</span><input type="number" name="answer" min="${e(q.min)}" max="${e(q.max)}" step="any" inputmode="decimal" required placeholder="Ange ett tal"></label>`;
  ...
  document.querySelector('#answer-form').addEventListener('submit',async ev=>{ev.preventDefault();const form=ev.target,button=document.querySelector('#send-answer');if(sending||done||!q.open||!form.reportValidity())return;error(document.querySelector('#answer-error'),'');let value;
   ...
   if(q.kind==='number'&&!Number.isFinite(value)){error(document.querySelector('#answer-error'),'Ange ett tal inom frågans intervall.');return;}
...
async function livePage(){
 ...
 try{if(bootError)throw Error(bootError);const first=await api('results',{code});q=first.question;answers=first.answers.map(a=>a.value);lastId=first.answers.at(-1)?.id||0;}catch(err){app.innerHTML=`<section class="panel joincard"><h1>Kunde inte öppna resultatet</h1><div class="error" role="alert">${e(err.message)}</div><a class="btn" href="${urlFor('live',code)}">Försök igen</a><a class="btn" href="./">Till startsidan</a></section>`;return;}
