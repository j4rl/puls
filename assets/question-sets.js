...
export async function liveQuestionSet({app,api,code,initial,settings,isStopped}){
 ...
 find('#set-result-question').addEventListener('change',async event=>{
  selectedId=Number(event.target.value);loading=true;lastId=0;answers=[];draw();await refresh(true);
 });
