...
function find_set_membership(string $code): ?array {
    return query('SELECT m.set_id,root.code AS root_code FROM puls_questions q JOIN puls_set_questions m ON m.question_id=q.id JOIN puls_questions root ON root.id=m.set_id WHERE q.code=?','s',[$code])->get_result()->fetch_assoc() ?: null;
}
...
function set_questions(int $id): array {
    return query('SELECT q.*,m.position FROM puls_set_questions m JOIN puls_questions q ON q.id=m.question_id WHERE m.set_id=? ORDER BY m.position','i',[$id])->get_result()->fetch_all(MYSQLI_ASSOC);
}
...
function set_answered_ids(int $id): array {
    $rows=query('SELECT a.question_id FROM puls_answers a JOIN puls_set_questions m ON m.question_id=a.question_id WHERE m.set_id=? AND a.voter_hash=?','is',[$id,voter_hash()])->get_result()->fetch_all(MYSQLI_ASSOC);
...
function handle_question_set(string $action,string $code,int $id): void {
    ...
    try {
        ...
        $set=query('SELECT * FROM puls_question_sets WHERE id=?','i',[$id])->get_result()->fetch_assoc();
        ...
        if ($action==='set-design') {
            ...
            $rows=query('SELECT id,value_json FROM puls_answers WHERE question_id=? AND id>? ORDER BY id LIMIT 5000','ii',[$questionId,$since])->get_result()->fetch_all(MYSQLI_ASSOC);
            $counts=query('SELECT m.question_id,COUNT(a.id) AS n FROM puls_set_questions m LEFT JOIN puls_answers a ON a.question_id=m.question_id WHERE m.set_id=? GROUP BY m.question_id','i',[$id])->get_result()->fetch_all(MYSQLI_ASSOC);
            ...
            if (in_array($questionId,$answered,true)) $result=['ok'=>true,'already'=>true];
                ...
                $count=query('SELECT COUNT(*) AS n FROM puls_answers WHERE question_id=?','i',[$questionId])->get_result()->fetch_assoc();
                if ((int)$count['n']>=max(1,min(5000,(int)($config['max_answers']??5000)))) {$db->rollback();respond(['error'=>'Frågan har nått sin svarsgräns.'],409);}
                query('INSERT INTO puls_answers (question_id,voter_hash,value_json) VALUES (?,?,?)','iss',[$questionId,voter_hash(),json_encode($value,JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR)]);
                $result=['ok'=>true];$status=201;
            }
        } elseif ($action==='update') {
            if (isset($data['open']) && is_bool($data['open']) && !(bool)$set['finished']) query('UPDATE puls_question_sets SET is_open=? WHERE id=?','ii',[$data['open']?1:0,$id]);
            else {
                $selected=null;
                ...
                query('UPDATE puls_questions SET result_view=? WHERE id=?','si',[$data['view'],(int)$selected['id']]);
            }
            $result=['ok'=>true];
            ...
            if ((int)$set['current_position']+1<count($questions)) {
                ...
                query('UPDATE puls_question_sets SET current_position=? WHERE id=?','ii',[$set['current_position'],$id]);
            } else {
                $set['finished']=1;$set['is_open']=0;
                query('UPDATE puls_question_sets SET finished=1,is_open=0 WHERE id=?','i',[$id]);
            }
            ...
            query('DELETE FROM puls_questions WHERE id IN ('.implode(',',array_fill(0,count($ids),'?')).')',str_repeat('i',count($ids)),$ids);
            $result=['ok'=>true];
        }
